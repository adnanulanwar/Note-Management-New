...
namespace NoteService.Controllers
{
    ...
    public class NoteController : ControllerBase
    {
        ...
        [Authorize]
        public List<Note> GetNotesByType([FromBody] Note searchNote)
        {
            ...
            foreach (var row in lines)
            {
                ...
                note.NoteTypeInt = int.Parse(row.Split("~~")[2]);
            ...
            notes = notes.Where(x => x.UserID == searchNote.UserID && x.NoteTypeInt == searchNote.NoteTypeInt).ToList();
            return notes;
        }

        // GET api/<NoteController>/5
        [HttpGet]
        public Note Get([FromBody] Note searchNote)
        {
            List<Note> notes = new List<Note>();
            ...
            Note rnote = new Note();
            rnote = notes.FirstOrDefault(x => x.ID == searchNote.UserID && x.UserID == searchNote.UserID);
            return rnote;
        }
        ...
        {
            if(note.NoteTypeInt == (int) EnumNoteType.Reminder || note.NoteTypeInt == (int)EnumNoteType.Task)
            {
            ...
            string noteStr = note.ID + "~~" +note.NoteMessage + "~~" + note.NoteTypeInt + "~~" +note.UserID + "~~" +note.NoteCreateTime + "~~" + note.ReminderTime;
